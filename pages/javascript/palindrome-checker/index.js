...
import palindromeChecker from '../../../public/js/palindromeChecker';
import {palindromeCheckerMetaData} from '../../../components/js/metaData';

export default function Home() { 
  let [inputText, setInputText] = useState("");
  let [highlight, setHighlight] = useState(false);
  let iconClass = "fas fa-undo";
  ...
  let metaData = {
    ...palindromeCheckerMetaData, checkList, highlight, handleHighlight, iconClass
  };
  ...
  return (
    <AppLayout metaData={metaData}>
      ...
      <div className="options mt-3 d-flex flex-column align-items-end">
        <button type="button" className="btn btn-danger border-2 mt-2" id="runButton" onClick={() => {handleHighlight(false)}} >Palindrome Check</button>
        <button type="button" className="btn btn-secondary border-2 mt-2" id="resetButton" onClick={() => {
            handleHighlight(false);
            setInputText("");
          }} >Clear</button>
      </div>
      <div className="alert mt-4 invisible fade show" id="result" role="alert">
        <h5>Result</h5>
        <hr />
